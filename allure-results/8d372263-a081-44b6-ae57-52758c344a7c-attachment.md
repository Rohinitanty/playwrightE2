...
```ts
  ...
  5  | 
  6  | test('Create and Search Lead Test', async ({ page }) => {
  7  | 
  ...
  14 | 
  15 |     // Wait after login (important for stability)
  16 |     await page.waitForLoadState('networkidle');
  17 | 
  18 |     // ---------- Create Lead ----------
> 19 |     const leadsPage = new CreateLeads(page);
     |                       ^ TypeError: _CreateLeads.CreateLeads is not a constructor
  20 | 
  21 |     await leadsPage.goToLeads();
  22 | 
  23 |     await leadsPage.enterLeadName("Rohini");
  24 |     await leadsPage.enterCompanyName("Qspider");
  25 |     await leadsPage.enterLeadSource("Online");
  26 |     await leadsPage.enterIndustry("IT");
  27 |     await leadsPage.enterPhone("1010101010");
  28 |     await leadsPage.enterLeadStatus("New");
  29 | 
  30 |     await leadsPage.selectFirstCampaign();
  31 |     await leadsPage.clickCreateLead();
  32 | 
  33 |     // Better verification than body check
  34 |     await expect(page.getByText("Rohini").first()).toBeVisible();
  35 | 
  36 |     // ---------- Search Lead ----------
  37 |     const updateLead = new UpdateLead(page);
  38 | 
  39 |     await updateLead.goToLeads();
  40 | 
  41 |     await updateLead.selectLeadByName("Rohini");
  42 | 
  43 |     // verify lead is visible in results
  44 |     await expect(page.getByText("Rohini").first()).toBeVisible();
  45 | });
```