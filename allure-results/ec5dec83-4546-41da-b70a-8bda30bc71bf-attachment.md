...
```
Error: locator.click: Error: strict mode violation: locator('div').filter({ has: locator('text=Campaign') }).locator('button') resolved to 3 elements:
...
Call log:
  - waiting for locator('div').filter({ has: locator('text=Campaign') }).locator('button')
...
```ts
  ...
  43 |   }
  44 | 
  45 |   async enterCampaign() {
  46 |   const campaignSection = this.page.locator('div').filter({
  47 |     has: this.page.locator('text=Campaign')
  48 |   });
  49 | 
  50 |   const [popup] = await Promise.all([
  51 |     this.page.waitForEvent('popup'),
> 52 |     campaignSection.locator('button').click()
     |                                       ^ Error: locator.click: Error: strict mode violation: locator('div').filter({ has: locator('text=Campaign') }).locator('button') resolved to 3 elements:
  53 |   ]);
  54 | 
  55 |   await popup.waitForLoadState();
  56 |   await popup.getByRole('button', { name: 'Select' }).first().click();
