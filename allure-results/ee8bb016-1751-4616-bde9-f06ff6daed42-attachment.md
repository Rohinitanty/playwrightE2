...
Received string:  "http://49.249.28.218:8098/dashboard"
Timeout: 5000ms

Call log:
  - Expect "toHaveURL" with timeout 5000ms
    9 × unexpected value "http://49.249.28.218:8098/dashboard"
...
```ts
  ...
  6  |   }
  7  | 
  8  |   // FIX 1: force correct navigation
  9  |   async goToLeads() {
  10 |     await this.page.goto('http://49.249.28.218:8098/leads');
  11 | 
> 12 |     await expect(this.page).toHaveURL(/leads/i);
     |                             ^ Error: expect(page).toHaveURL(expected) failed
  13 | 
  14 |     // optional UI validation
  15 |     await expect(this.page.locator('body')).toContainText(/lead/i);
  16 |   }
  17 | 
  18 |   async openCreateForm() {
  19 |     //  FIX 2: correct button detection
  20 |     const createBtn = this.page.locator('button:has-text("Create")').first();
  21 | 
  22 |     await expect(createBtn).toBeVisible({ timeout: 15000 });
  23 |     await createBtn.click();
  24 | 
  25 |     // wait for create form/modal
  26 |     await expect(
  27 |       this.page.locator('text=/create/i')
  ...
  67 |     const dropdown = this.page.locator('text=Campaign').locator('..').locator('button').first();
  68 | 
  69 |     await expect(dropdown).toBeVisible();
  70 |     await dropdown.click();
  71 | 
  ...
  78 |     const submitBtn = this.page.locator('button:has-text("Create")').last();
  79 | 
  80 |     await expect(submitBtn).toBeEnabled({ timeout: 15000 });
  81 |     await submitBtn.click();
  82 | 
  83 |     // FIX 3: wait for result, not URL assumption
  84 |     await expect(this.page.locator('body')).toContainText(/lead/i);
  85 |   }
  86 | }
```