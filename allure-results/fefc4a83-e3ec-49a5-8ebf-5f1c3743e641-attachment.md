...
=========================== logs ===========================
waiting for navigation to "**/leads" until "load"
============================================================
```
...
```ts
  ...
  31 | 
  32 |   // wait for leads page
> 33 |   await page.waitForURL('**/leads');
     |              ^ Error: page.waitForURL: Test timeout of 30000ms exceeded.
  34 | 
  35 |   // ✅ correct verification (table cell)
  36 |   await expect(
  37 |     page.getByRole('cell', { name: 'Rohini' })
  38 |   ).toBeVisible();
  39 | 
  40 |   // SEARCH + EDIT
  41 |   const updateLead = new UpdateLead(page);
  42 | 
  43 |   await updateLead.goToLeads();
  44 |   await updateLead.selectLeadByName("Rohini");
  45 |   await updateLead.clickEditIconForLead();
  46 | });
```