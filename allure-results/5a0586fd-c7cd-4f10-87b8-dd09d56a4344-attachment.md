...
- Name: ProjectNinjaCRM\NinjaCRM.spec.js >> Create Lead Test
...
```ts
  ...
  22 | 
  23 |     //  Assertion (verify lead created)
> 24 |     await expect(page.getByText("Rohini")).toBeVisible();
     |                                            ^ Error: expect(locator).toBeVisible() failed
  25 | 
  26 | });
```