...
Locator: getByRole('button', { name: /Create Lead/i })
Expected: enabled
Timeout: 5000ms
...
```ts
  ...
  12 | 
  13 |     // wait for leads page to stabilize
  14 |     await expect(this.page.getByText(/Leads/i).first()).toBeVisible();
  15 |   }
  16 | 
  17 |   async openCreateForm() {
  18 |     const createBtn = this.page.getByRole('button', { name: /Create Lead/i });
  19 | 
  20 |     await expect(createBtn).toBeVisible();
  21 |     await createBtn.click();
  22 | 
  23 |     // IMPORTANT: wait for form instead of button
  24 |     await expect(this.page.getByRole('heading', { name: /Create Lead/i }))
  25 |       .toBeVisible();
  26 |   }
  27 | 
  28 |   async fillField(label, value) {
  29 |     await this.page
  30 |       .locator(`text=${label}`)
  31 |       .locator('..')
  32 |       .locator('input')
  33 |       .first()
  34 |       .fill(value);
  35 |   }
  36 | 
  37 |   async enterLeadName(name) {
  38 |     await this.fillField('Lead Name', name);
  39 |   }
  40 | 
  41 |   async enterCompanyName(company) {
  42 |     await this.fillField('Company', company);
  43 |   }
  44 | 
  45 |   async enterLeadSource(source) {
  46 |     await this.fillField('Lead Source', source);
  47 |   }
  48 | 
  49 |   async enterIndustry(industry) {
  50 |     await this.fillField('Industry', industry);
  51 |   }
  52 | 
  53 |   async enterPhone(phone) {
  54 |     await this.fillField('Phone', phone);
  55 |   }
  56 | 
  57 |   async enterLeadStatus(status) {
  58 |     await this.fillField('Lead Status', status);
  59 |   }
  60 | 
  61 |   async selectFirstCampaign() {
  62 |     const campaignBtn = this.page
  63 |       .locator('text=Campaign*')
  64 |       .locator('..')
  65 |       .locator('button');
  66 | 
  67 |     await expect(campaignBtn).toBeVisible();
  68 |     await campaignBtn.click();
  69 | 
  70 |     const option = this.page.locator('text=/CAM/i').first();
  71 |     await expect(option).toBeVisible();
  72 |     await option.click();
  73 |   }
  74 | 
  75 |   async clickCreateLead() {
  76 |     const submitBtn = this.page.getByRole('button', { name: /Create Lead/i });
  77 | 
> 78 |     await expect(submitBtn).toBeEnabled();
     |                             ^ Error: expect(locator).toBeEnabled() failed
  79 |     await submitBtn.click();
  80 | 
  81 |     // wait for success navigation OR table update
