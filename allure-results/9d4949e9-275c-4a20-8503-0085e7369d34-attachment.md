...
```ts
  ...
  7  |         this.createLeadBtn = page.getByRole("button", { name: "Create Lead" });
  8  |         this.leadNameInput = page.locator("input[name='name']");
  9  |     }
  10 | 
  11 |     async goToLeads() {
  12 |         await this.leadsLink.click();
  13 |         await this.createLeadBtn.click();
  14 |     }
  15 | 
  16 |     async enterLeadName(name) {
> 17 |         await expect(this.leadNameInput).toBeVisible();
     |         ^ ReferenceError: expect is not defined
  18 |         await this.leadNameInput.fill(name);
  19 |     }
  20 | }
```