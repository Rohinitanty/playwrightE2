...
Call log:
  - waiting for getByRole('button', { name: /Create Lead/i }) to be visible
...
```ts
  ...
  19 |     await this.createBtn.click();
  20 |   }
  21 | 
  22 |   async fillField(label, value) {
  23 |     await this.page
  24 |       .locator(`text=${label}`)
  25 |       .locator('..')
  26 |       .locator('input')
  27 |       .first()
  28 |       .fill(value);
  ...
  57 |     const campaign = this.page.locator('text=Campaign*').locator('..').locator('button');
  58 | 
  59 |     await campaign.click();
  60 | 
  61 |     const option = this.page.locator('text=/CAM/i').first();
  62 |     await option.waitFor({ state: 'visible' });
  63 |     await option.click();
  64 |   }
  65 | 
  66 |   async clickCreateLead() {
  67 |     const btn = this.page.getByRole('button', { name: /Create Lead/i });
  68 | 
> 69 |     await btn.waitFor({ state: 'visible' });
     |               ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  70 |     await btn.click();
  71 | 
  72 |     // wait for UI update instead of navigation
  73 |     await this.page.waitForTimeout(1500);
  74 |   }
  75 | }
```